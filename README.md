...
This is still a work in progress.  The verify signature function uses about 10 million gas while the update public key is around 6 million so anyone updating their key in a falcon-signed transaction may have issues with the ~16 million transaction limit introduced with Fukasa.  To reduce gas consumption on verification, the public keys are transformed ready for Montgomery multiplication when loaded so they do not need to be transformed with each verification.  I am considering changing the update key function to assume that Montgomery multiplication has occurred off-chain.  I may also simplify the key decoding function to simple bytes like the signature instead of the big-endian/little-endian conversion.