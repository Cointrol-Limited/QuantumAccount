...
contract QuantumAccountTest is Test {
    ...
    function setUp() external {
        ...
        (, publicKey) = mockData.getSignatureAndPublicKey(0);
    ...
    function testUpdatePublicKeyViaEntryPoint() public {
        // Arrange
        (, bytes memory newPublicKey) = mockData.getSignatureAndPublicKey(1);
        //Act
        vm.prank(address(entryPoint));
        quantumAccountHarness.updatePublicKey(newPublicKey);
        //assert
        vm.assertEq(quantumAccountHarness.getPublicKeyBytes(), newPublicKey);
        }

    function testUpdatePublicKeyRevertsWhenNotFromEntryPoint() public {
        // Arrange
        (, bytes memory newPublicKey) = mockData.getSignatureAndPublicKey(1);
